...
--EXEMPLO:
--EXECUTE SP_GERAR_PEDIDO 3, 5, '14,7', '1,1'
--GO
...
BEGIN 
	...
	ELSE
		BEGIN
			SET @NUMVENDA = 1
		END
	IF NOT EXISTS (SELECT TOP 1 ID_CLIENTE FROM CLIENTE WHERE ID_CLIENTE = @IDCLIENTE)
		BEGIN
	...
	IF(SELECT COUNT(VALUE) FROM STRING_SPLIT(@QTDE,',',1)) <> (SELECT COUNT(VALUE) FROM STRING_SPLIT(@PRODUTOS, ',',1))
		...
		WHILE(LEN(REPLACE(@QTDE,',','')) >= @CONT)
			BEGIN
				...
				IF NOT EXISTS(SELECT TOP 1 VALOR FROM PRODUTO WHERE ID_PRODUTO = @IDPRODUTO)
					BEGIN
						PRINT 'PRODUTO INEXISTENTE'
						ROLLBACK
						RETURN
					END 
				IF(SELECT QTD FROM PRODUTO WHERE ID_PRODUTO = @IDPRODUTO) < @VAR
					BEGIN
						PRINT 'QUANTIDADE SUPERIOR A QUE ESTOQUE POSSUI'
						ROLLBACK
						RETURN
					END
				SET @VALOR_UNIT = (SELECT VALOR FROM PRODUTO WHERE ID_PRODUTO = @IDPRODUTO)
				SET @VALOR_TOT = @VALOR_UNIT * @VAR
				

				INSERT INTO PEDIDO_VENDA(COD_VENDA, COD_SEQ, ID_VENDEDOR, ID_PRODUTO, ID_CLIENTE, QTD, VALOR_UNITARIO, VALOR_TOTAL, STATUS_PEDITO)
				VALUES (@NUMVENDA,@NUMSEQ,@IDVENDEDOR,@IDPRODUTO, @IDCLIENTE, @VAR, @VALOR_UNIT, @VALOR_TOT, 'P')
...
IF EXISTS (SELECT COD_VENDA FROM VENDA WHERE COD_VENDA = @COD)
BEGIN
...
BEGIN
	...
	UPDATE PRODUTO
	SET QTD = PRODUTO.QTD - (SELECT QTD FROM #TESTE WHERE COD_SEQ = @CODSEQ)
...
BEGIN
	IF NOT EXISTS(SELECT TOP 1 COD_VENDA FROM PEDIDO_VENDA WHERE COD_VENDA = @COD)
		BEGIN
			...
			RETURN 
		END
	IF(SELECT TOP 1 STATUS_PEDITO FROM PEDIDO_VENDA WHERE COD_VENDA = @COD) = 'APROVADO'
		BEGIN
			PRINT 'O CODIGO DO PEDIO INFORMADO JÁ FOI APROVADO'
			RETURN
		END
	IF(SELECT TOP 1 STATUS_PEDITO FROM PEDIDO_VENDA WHERE COD_VENDA = @COD) = 'CANCELADO'
		BEGIN
